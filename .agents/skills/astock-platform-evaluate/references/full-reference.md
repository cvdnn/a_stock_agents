...
```bash
cp .agents/skills/astock-platform-evaluate/scripts/monitor_watchdog.py output/scripts/
...
# 输出: output/reports/20260729/aStocks_600519_20260729.html
```
...
| 项目 | 约定 | 说明 |
|:-----|:-----|:-----|
| **基准目录** | `output/reports/<YYYYMMDD>/` | 以日期分目录，自动创建 |
...
| **模板路径** | `.agents/skills/astock-data-feed/templates/stock-report.html` | 含完整CSS变量+组件系统 |
...
```bash
# 1. 读取模板
TEMPLATE=".agents/skills/astock-data-feed/templates/stock-report.html"
HTML=$(cat "$TEMPLATE")
...
# 3. 确定输出路径
DATE_DIR="output/reports/$(date +%Y%m%d)"
mkdir -p "$DATE_DIR"
...
export ASTOCKS_A_SHARE_DATA_DIR=".agents/skills/astock-data-feed"
```
...
## 十四、Python 代码集成
import sys
sys.path.insert(0, ".agents/skills/astock-platform-evaluate/scripts")
...
### 4. 详细实战手册与量化规则库
完整操作规则、量化阈值与决策公式详见参考文档：[`action-execution-manual.md`](action-execution-manual.md)
