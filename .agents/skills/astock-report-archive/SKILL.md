...
version: "1.0.0"
author: ""
description: A股报告持久化与归档规范 — 输出路径约定、多股联合报告模式、Tencent API+Python自算技术指标降级方案
tags: [A股, 报告, 持久化, 格式规范]
related_skills: [astock-report-html, astock-platform-evaluate, astock-data-feed]
---
...
```
<项目根目录>/output/reports/<YYYYMMDD>/<报告类型>_<描述>.<ext>
```

默认路径: 由 `core.config.REPORTS_DIR` 动态解析（默认指向 `output/reports/<YYYYMMDD>/`）
