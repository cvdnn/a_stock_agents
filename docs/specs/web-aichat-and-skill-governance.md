...
### 5.1 双模动态视口与 AI 助手灵活定位规范
- **意图驱动双模架构**：
  1. **【投研助手】模式**：左侧点击【投研助手】时，【AIChatUI】居于中间主导位（`grid-template-columns: 240px 4fr 6fr;`），右侧为辅助盘面与动作单投射区；
  2. **【业务主工作区】模式**：点击【市场行情】、【自选个股】、【收益分析】等功能时，业务区居于中间跃升为主工作区（`grid-template-columns: 240px 1fr 390px;`），【AIChatUI】定位为伴随式 AI 助手并变换至右侧；
- **AI助手收起与 Title 右侧常驻小按钮**：
  - 在右侧 AI 助手点击 `[▶ 收起]` 后，右侧平滑收敛隐藏（宽度 0），中间业务主工作区扩充为 **100% 全宽**；
  - 中间主工作区 Title 栏右侧展示小按钮 `[🤖 AI助手]`，点击可重新在右侧平滑展开 AI 助手；
- **视口管理**：高度严格锁定在 `calc(100vh - 50px); overflow: hidden;`，彻底收敛全局外层滚动，且每次视口切换均自动触发图表分辨率重绘。
