...
| 建立前端结构、交互、无 AI 与失败恢复测试 | `tests/frontend/` | 🟨 进行中（矩阵 1–5 已覆盖） |
...
| 手动同步创建真实 `data_sync` 任务并可跟踪 | `tests/frontend/`、`tests/server/` | 🟨 前端已验证，后端待实施 |
| 活动任务可取消，失败任务可重试 | `tests/frontend/`、`tests/server/` | 🟨 前端已验证，后端待实施 |
...
| P3 手动任务支持市场/代码范围选择，全量模式必须二次确认 | `tests/frontend/`、`tests/server/` | 🟨 前端已验证，后端待实施 |
| P3 单任务互斥且不得抢占 P0/P1 任务 | `tests/server/`、`tests/core/` | ⬜ 待验证 |
| 新任务统一创建为 `data_sync`，历史 `sync` 记录仍可读取 | `tests/frontend/`、`tests/server/` | 🟨 前端已验证，后端待实施 |
