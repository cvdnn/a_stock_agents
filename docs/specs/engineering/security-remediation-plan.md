...
- [x] **目标文件**：`web/js/chat_presentation.js`
- [x] **实施步骤**：
  1. 修改 `sanitizeHtmlOutput`：移除 `ADD_ATTR` 白名单中的 `'onclick'`。
  2. 对前端快捷操作按钮（如实战动作单、查看详情），改用 `data-action` 属性并使用事件监听委托。
  3. 修改未加载 DOMPurify 时的 fallback 分支：采用全局危险标签与事件过滤规则（剥离所有 `on*` 事件属性与伪协议），杜绝黑名单绕过与转义损坏。
...
- [x] **目标文件**：`scripts/server/agent/tools.py`, `scripts/core/reporting/report_generator.py`
- [x] **实施步骤**：
  1. 在 `generate_simple_report(data, output_path)` 中对 `output_path` 实施约束：引入 `_sanitize_report_output_path` 强制收敛至 `output/reports` 或 `OUTPUT_REPORTS_DIR`。
...
- [x] **目标文件**：`docker-compose.yml`, `scripts/server/config.py`, `scripts/server/app.py`
- [x] **实施步骤**：
...
- [x] **目标文件**：`scripts/server/api/models_mgmt.py`
- [x] **实施步骤**：
...
#### 9. 安全自动化回归测试套件 (TEST)
- [x] **目标文件**：`tests/test_security_audit.py`
- [x] **实施步骤**：
