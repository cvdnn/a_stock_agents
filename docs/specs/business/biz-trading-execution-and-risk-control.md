...
- **权威设计指南**：[`docs/guidelines/business/trading-execution-rules.md`](../../guidelines/business/trading-execution-rules.md)

> 🔗 **权威规范与规则定义直达**：  
> 本文件为 **实战交易反应动作与三级风控止损阶梯实施落地与任务执行跟踪看板**。关于阿尔法与贝塔解耦分层架构、实战交易三原则、六大交易反应动作、量化风控阈值库等完整规范，请查阅权威指南：  
> 👉 [**《A股实战交易反应动作与量化决策风控执行规则》(trading-execution-rules.md)**](../../guidelines/business/trading-execution-rules.md)
...
- **关键设计要点**：
  1. [阿尔法选股与贝塔执行解耦](../../guidelines/business/trading-execution-rules.md#一-核心架构原则阿尔法选股与贝塔执行彻底解耦)：第一层负责打分选股，第二层执行中枢（EMS）负责翻译动作指令，第三层负责终端呈现；
  2. [实战交易三原则（输出铁律）](../../guidelines/business/trading-execution-rules.md#二-实战交易三原则智能体输出铁律)：最低保本卖出价（强制向上进位）、三级风控止损阶梯（T0 -3% / T1 -5% / T2 -8%）、三场景即时动作单；
  3. [六大交易反应动作标准化](../../guidelines/business/trading-execution-rules.md#三-六大交易反应动作操作规范)：突破建仓40/60、均线回踩挂单、假摔反向做T、顺势动态止盈、弱势反弹减仓、破位开盘避险；
  4. [量化监控门禁阈值库](../../guidelines/business/trading-execution-rules.md#四-核心价量指标量化阈值库)：量比、MA20乖离率、换手率沉淀与 ATR 波动范围统一标准。
...
- **2026-09-08 (v1.3)**：按规范治理要求重构，将实战交易规则与操作定义抽离至 `docs/guidelines/business/trading-execution-rules.md`，本文件重塑为实施看板。
