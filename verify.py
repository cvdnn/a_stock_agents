...
def run_tests():
    ...
    print("[1/11] 检查技能清单与 SKILL.md 完整性...")
    try:
    ...
    print("[2/11] 测试行情数据层 (DataBridge & 腾讯实时快照)...")
    try:
    ...
    print("[3/11] 测试技术指标计算 (MA/MACD/KDJ/RSI/BOLL/ATR/二次金叉)...")
    try:
    ...
    print("[4/11] 测试交易反应动作引擎与精确保本价进位法则...")
    try:
    ...
    print("[7/11] 测试用户专属 Output 目录隔离与自定义配置动态生效...")
    try:
        import subprocess
        ...
        test_temp_out = PROJECT_ROOT / "temp" / "_verify_custom_output"
    ...
    print("[8/11] 检查核心模块全量导入与语法完整性 (Smoke Test)...")
    try:
        import importlib
    ...
    print("[9/11] 检查 SSOT 薄转发器与单一真理来源完整性...")
    try:
        import subprocess
        from pathlib import Path
    ...
    print("[10/11] 测试全功能 CLI 一体化调度能力 (screen, trapped, report)...")
    try:
        import subprocess
    ...
    try:
        ...
        scan_roots = [skills_p, PROJECT_ROOT / "scripts" / "core"]
        for scan_root in scan_roots:
            for candidate in scan_root.rglob("*"):
                ...
                for keyword in dirty_keywords:
                    if keyword in text:
                        dirty_found.append(f"{candidate.relative_to(PROJECT_ROOT)}: {keyword}")
        assert len(dirty_found) == 0, f"Dirty absolute paths found in skill files: {dirty_found}"
