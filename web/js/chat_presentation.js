(function (root) {
  ...
  function inline(text, state) {
    ...
    var placeholders = state.placeholders;
    ...
    out = out.replace(/\[([^\]]+)\]\(([^)\s]+)(?:\s+(?:"[^"]*"|&quot;[^&]*&quot;))?\)/g, function (_, label, url) {
      var safe = safeUrl(url.replace(/&amp;/g, '&')); return safe ? hold('<a href="' + escapeHtml(safe) + '" target="_blank" rel="noopener noreferrer">' + inline(label, state) + '</a>') : inline(label, state);
    });
    out = out.replace(/\*\*([^*\n]+)\*\*|__([^_\n]+)__/g, function (_, a, b) { return '<strong>' + (a || b) + '</strong>'; });
    out = out.replace(/\*([^*\n]+)\*|_([^_\n]+)_/g, function (_, a, b) { return '<em>' + (a || b) + '</em>'; });
    function resolve(value) {
      return String(value).replace(/\uE000(\d+)\uE001/g, function (_, i) { return resolve(placeholders[Number(i)] || ''); });
    }
    return resolve(out);
  }
  ...
  function renderMarkdown(markdown) {
    ...
    while (i < lines.length) {
      ...
      var para = [line]; i++; while (i < lines.length && lines[i].trim() && !/^\s*(#{1,6})\s+|^\s*[-*+]\s+|^\s*\d+[.)]\s+|^\s*>|^\s*```|^\s*\|?.+\|.+\|?\s*$/.test(lines[i])) para.push(lines[i++]); html.push('<p>' + inline(para.join('\n')).replace(/\n/g, '<br>') + '</p>');
    }
    return html.join('\n');
  }
  ...
  function redactSensitive(value) {
    return String(value == null ? '' : value)
      ...
      .replace(/(["']Authorization["']\s*:\s*["'])(?:Bearer|Basic)\s+[^"']*(["'])/gi, '$1[REDACTED]$2')
      .replace(/(Cookie\s*:\s*)[^\r\n]+/gi, '$1[REDACTED]')
      .replace(/(["'](?:api[_-]?key|token|cookie|secret)["']\s*:\s*["'])(?:\\["']|[^"'])*(["'])/gi, '$1[REDACTED]$2')
      .replace(/((?:api[_-]?key|token|cookie|secret)\s*[:=]\s*)("[^"]*"|'[^']*'|[^\s,;]+)/gi, '$1[REDACTED]');
  }
