(function (root) {
  ...
  function applyEvent(state, type, payload) {
    ...
    if (type === 'thought') {
      ...
      var terminal = String(eventValue(payload, ['status', 'state', 'outcome', 'type'], '')).toLowerCase(), toolError = payload.error || (payload.data && typeof payload.data === 'object' && (payload.data.code || payload.data.error || payload.data.message) ? payload.data : null) || (terminal === 'error' || terminal === 'timeout' ? payload : null);
      if (toolError || terminal === 'failed') { var wasFailed = match.status === 'failed', errorDetail = toolError && (toolError.detail || toolError.message || (typeof toolError === 'string' ? toolError : '')); match.status = 'failed'; if (!hasData) { match.result = null; if (match.callId != null) delete state.toolResultsByCallId[match.callId]; } match.error = presentError(toolError && toolError.code ? toolError : { code: terminal === 'timeout' ? 'LLM_TIMEOUT' : 'UNKNOWN', detail: errorDetail || payload.detail }); if (!wasFailed && !state._toolFailureSeen) { match.expanded = true; state.timelineExpanded = true; state._toolFailureSeen = true; } }
