(function (root) {
  ...
  function inline(text) {
    ...
    out = out.replace(/`([^`\n]+)`/g, function (_, x) { return hold('<code>' + escapeHtml(x) + '</code>'); });
    out = out.replace(/!\[([^\]]*)\]\([^)]*\)/g, function (_, alt) { return escapeHtml(alt); });
    out = out.replace(/\[([^\]]+)\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g, function (_, label, url) {
      var safe = safeUrl(url); return safe ? hold('<a href="' + escapeHtml(safe) + '" target="_blank" rel="noopener noreferrer">' + label + '</a>') : label;
    });
  ...
  function stripMarkdown(line) {
    return String(line).replace(/^\s*(?:[-*+] |\d+[.)] )/, '').replace(/^\s*#{1,6}\s*/, '').replace(/[`*_~]/g, '').replace(/\[([^\]]+)\]\([^)]*\)/g, '$1').replace(/\s+/g, ' ').trim();
  }
  function summarizeMarkdown(markdown, limit) {
    ...
    lines.forEach(function (line) { if (/^\s*```/.test(line)) { inFence = !inFence; return; } if (inFence) return; var h = line.match(/^\s*#{1,6}\s+(.+)/), clean = stripMarkdown(line); if (h) { active = wanted.test(stripMarkdown(h[1])); return; } if (clean) { if (active && (/^[-*+ ]|^\d+[.)]/.test(line) || clean.length > 8)) candidates.push(clean); else fallback.push(clean); } });
    var result = [], seen = {}; candidates.concat(fallback).forEach(function (x) { x = x.slice(0, 120); if (x && !seen[x] && result.length < limit) { seen[x] = true; result.push(x); } }); return result.slice(0, Math.min(limit, 5));
  }
  function redactSensitive(value) {
    return String(value == null ? '' : value).replace(/(Authorization\s*:\s*Bearer\s+)[^\s,;]+/gi, '$1[REDACTED]').replace(/((?:api[_-]?key|token|cookie|secret)\s*[:=]\s*)[^\s,;]+/gi, '$1[REDACTED]');
  }
  var errors = { LLM_NOT_CONFIGURED: ['模型未配置', '请先完成模型配置后重试'], LLM_AUTH_FAILED: ['模型认证失败', '请检查 API 密钥后重试'], LLM_MODEL_UNAVAILABLE: ['模型暂不可用', '请稍后重试或选择其他模型'], LLM_CAPABILITY_UNSUPPORTED: ['模型不支持此能力', '请更换支持该能力的模型'], LLM_TIMEOUT: ['请求超时', '请稍后重试'], SSE_HTTP_ERROR: ['服务连接失败', '请稍后重试'], SSE_INCOMPLETE: ['响应未完成', '请重试'] };
  function presentError(error) { var code = error && error.code, item = errors[code] || ['请求失败', '请稍后重试']; return { title: item[0], recovery: item[1], detail: redactSensitive(error && (error.detail || error.message || '')) }; }
