(function (root) {
  ...
  function summarizeMarkdown(markdown, limit) {
    ...
    var result = [], seen = {}; (candidates.length ? candidates : fallback).forEach(function (x) { x = x.slice(0, 120); if (x && !/^[-| ]+$/.test(x) && !seen[x] && result.length < limit) { seen[x] = true; result.push(x); } }); return result.slice(0, Math.min(limit, 5));
  }
  function redactSensitive(value) {
    return String(value == null ? '' : value).replace(/(Authorization\s*:\s*Bearer\s+)[^\s,;]+/gi, '$1[REDACTED]').replace(/(Cookie\s*:\s*)[^\r\n]+/gi, '$1[REDACTED]').replace(/("(?:api[_-]?key|token|cookie|secret)"\s*:\s*")([^"]*)(")/gi, '$1[REDACTED]$3').replace(/((?:api[_-]?key|token|cookie|secret)\s*[:=]\s*)["']?[^\s,;"'}]+["']?/gi, '$1[REDACTED]');
  }
