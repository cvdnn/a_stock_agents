...
def cmd_limit_up_pool(date_str: str, top: int, output_json: bool):
    """涨停股池 — 使用 efinance 龙虎榜（零积分，数据更丰富）"""
    if not date_str:
        date_str = datetime.now().strftime("%Y-%m-%d")
    try:
        # efinance 的日期格式为 YYYY-MM-DD
        df = ef.stock.get_daily_billboard(date_str)
        if df is None or df.empty:
            print(f"未找到 {date_str} 的龙虎榜数据")
            return
        # 过滤涨停：涨跌幅 >= 9.9%
        df = df[df["涨跌幅"] >= 9.9].head(top).copy()
        keep = ["股票代码", "股票名称", "涨跌幅", "收盘价", "换手率",
                "龙虎榜净买额", "上榜原因"]
        available = [c for c in keep if c in df.columns]
        df = df[available].copy()
        df["龙虎榜净买额(万)"] = (df["龙虎榜净买额"] / 1e4).round(2)
        df = df.drop(columns=["龙虎榜净买额"])
        # 改列名兼容原有输出约定
        ...
        if output_json:
            ...
            print(f"【涨停股池】{date_str}  共 {len(df)} 只  数据源：efinance(龙虎榜)")
            print(df.to_string(index=False))
    except Exception as e:
        print(f"获取涨停股池失败：{e}")
        sys.exit(1)
...
def cmd_fund_flow(code: str, days: int, output_json: bool):
    """个股资金流向 — 使用 efinance（零积分，数据覆盖更全）"""
    try:
        df = ef.stock.get_history_bill(code)
        if df is None or df.empty:
            print(f"未找到 {code} 的历史资金流向数据")
            # 降级：尝试今日逐分钟资金流向
            try:
                df_today = ef.stock.get_today_bill(code)
        ...
        if output_json:
            ...
            print(f"【{code} 资金流向】近 {days} 日  数据源：efinance（单位：万元）")
            print(df.to_string(index=False))
    except Exception as e:
        print(f"获取资金流向失败：{e}")
        sys.exit(1)
...
def _load_auth_token():
    ...
    if os.path.exists(cfg_path):
        ...
        return (cfg.get("proxy_patch", {}).get("auth_token") or "")
    return ""


def cmd_balance(output_json: bool):
    """查询 proxy-patch 积分余额"""
    ...
    try:
        ...
        if output_json:
            ...
            print(f"【Proxy 积分余额】{balance} 积分")
            if balance < 100:
                print("⚠️  积分不足100，建议减少 proxy 调用或充值")
            elif balance < 1000:
                print("⚡  积分低于1000，注意节省使用")
    except Exception as e:
        print(f"查询积分余额失败: {e}")
        sys.exit(1)
...
def main():
    ...
    parser.add_argument("--balance", action="store_true", help="查询 proxy-patch 积分余额")
