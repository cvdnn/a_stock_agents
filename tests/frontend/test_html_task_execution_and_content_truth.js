...
const pyBin = process.env.PYTHON || (fs.existsSync(venvPython) ? venvPython : 'python');
const pyScript = "import sys; sys.path.insert(0, 'scripts'); from core.reporting.report_generator import wrap_markdown_as_html_report; md = '# 中国移动\\n| 指标 | 读数 |\\n|---|---|\\n| 现价 | 97.72 |'; html = wrap_markdown_as_html_report(md, title='中国移动研报', filename='aStocks_600941.html'); assert '<!DOCTYPE html>' in html; assert 'class=\"tbl\"' in html; assert not html.startswith('# 中国移动'); print('PYTHON_WRAP_SUCCESS')";
const pyOut = execFileSync(pyBin, ['-c', pyScript], {
  cwd: repoRoot,
  encoding: 'utf-8'
});
