...
for p in [PROJECT_ROOT, SCRIPTS_DIR, CUR_DIR]:
    ...
    if cmd == "quote":
        ...
        if server_cmd == "start":
            ...
            if getattr(args, "json", False):
                ...
                print(f"🚀 正在启动 A-Stock Agents Web 服务: http://{host}:{port}")
            ...
            try:
                ...
                if getattr(args, "json", False):
                    ...
                    print(f"✅ 服务运行正常: {base_url} (v{data.get('version')}, DB: {'已连接' if data.get('db_connected') else '未连接'})")
            except Exception as e:
                err = {"status": "offline", "error": str(e), "url": base_url}
                if getattr(args, "json", False):
