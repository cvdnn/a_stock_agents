...
def build_parser() -> argparse.ArgumentParser:
    ...
    backtest.add_argument("--strategy", default="sma_cross",
                          choices=["buy_and_hold", "sma_cross", "rsi_revert", "boll_reversion"])
    ...
    backtest.add_argument("--slippage-bps", type=float, default=10.0,
                          help="Slippage in basis points (default 10 = 0.1%)")
    backtest.add_argument("--no-t1", action="store_true",
                          help="Disable T+1 execution (use same-bar close fill)")
    return parser
    ...
    if cmd == "create-account":
        ...
        result = request_json(
            ...
            "/backtest",
            {
                ...
                "params": {"fast": args.fast, "slow": args.slow, "buy_rsi": args.buy_rsi, "sell_rsi": args.sell_rsi, "slippage_bps": args.slippage_bps, "t_plus_1": not args.no_t1},
            },
        )
    else:
